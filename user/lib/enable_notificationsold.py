...
class Enable_Notifications:
    ...
    def get_parsed_values(self, xmlData, tag_name):
        ...
        try:
            ...
            LOG.info('There is no notifications\n')
    ...
    def edit_config_opr(self):
        ...
        try:
            ...
            LOG.info(
                ...
                xmldata)
            nose.tools.assert_in('<ok/>', str(xmldata))
            return p

        except Exception as err:
            print "\n\n"
            LOG.error('-----[ Error from the Switch ]-----\n\n%s\n\n' % err)
            LOG.info('-----[ Getting Error from the Switch ]------ : FAIL')
            return f
    ...
    def get_notif_port(self, **kwargs):
        ...
        if result == 'PASS':
            ...
            if result == 'PASS':
                ...
                LOG.info('notif_name : %s' % kwargs['notif_name'])
                if str(kwargs['notif_name']) in result:
                    LOG.info('notif_names : %s' % result)
