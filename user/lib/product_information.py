...
class Product_Information:
    ...
    def compare_list(self, xmlData):
        ...
        for info in product_info:
            for i in range(0, len(product_information_list)):
                try:
                    info_tag = info.getElementsByTagName(
                        product_information_list[i])[0]
        ...
        if str(existing_product_info) == str(product_info_values):
            LOG.info(
                'compare both existing_product_info and required product_info : PASS\n')
            return p
        else:
    ...
    def get_manufacturer(self, **kwargs):
        ...
        product_info = Element(
            'opsw:product-information', {'xmlns:plts': "http://www.polatis.com/yang/polatis-switch",
	...
	prt_1 =get_config_arg("port_type", "prt_1")
        port_types = [prt_1, prt_1, prt_1]
    ...
    def get_ingress_ports_has_opm_info(self, **kwargs):
        ...
        ingress_port_ids = kwargs['ingress_port_ids']
	opm = get_config_arg("has_opm", "opm")
        has_opm_values = [opm, opm, opm]
    ...
    def get_ingress_ports_has_oxc_info(self, **kwargs):
        ...
        ingress_port_ids = kwargs['ingress_port_ids']
	oxc = get_config_arg("has_oxc", "oxc")
        has_oxc_values = [oxc, oxc, oxc]
	...
	prt_2 =get_config_arg("port_type", "prt_2")
        port_types = [prt_2, prt_2, prt_2]
        ...
        csvOutput(
            'product-information',
            'get_egress_ports_type_info',
            d,
            result)
        nose.tools.assert_equals('PASS', result)

    def get_ingress_and_egress_ports_type_info(self, **kwargs):
        """create xml to get egress ports type info then parse get manufacturer switch output xml
        , compare it with existing values.Finally save the output in final.csv file
        Arguments:
        file_name             : any file name
        ...
        egress_port_ids = kwargs['port_ids']
	prt_1 = get_config_arg("port_type", "prt_1")
	prt_2 = get_config_arg("port_type", "prt_2")
        port_types = [prt_2, prt_1, prt_2]
