...
class System_Config:
    ...
    def editconfig_create_user_with_password_and_group(self, **kwargs):
        ...
        if result == 'PASS':
            #LOG.info(
            #    '-----[ create xml for get-config operation with names ]-----\n\n')
            ...
            elif kwargs['operation'] == 'get-config':
                LOG.info('-----[ create xml for get-config operation ]-----\n\n%s' % self.prettify(xmlstr))
                xmlout, result = self.getconfig_rpc_request(
                    xmlstr, kwargs['file_name'])
            else:
                LOG.info('Please provide valid operation - get/get-config')
            ...
            csvOutput(
                'system-config',
                'editconfig_create_user_with_password_and_group',
                d,
                result)
        else:
    ...
    def editconfig_create_user_with_group_and_invalid_password(self, **kwargs):
        ...
        xmlstr = self.create_xml_for_user_with_password_and_group()
        result, message = self.edit_config_opr()
        LOG.info('gvn_name : %s' % gvn_names)
        LOG.info('gvn_password : %s\n\n' % gvn_passwords)
        LOG.info('gvn_group : %s\n\n' % gvn_groups)
        if result == 'PASS':
            e = time.time()
            d = int(round((e - s) * 1000))
            ...
            csvOutput(
                'system-config',
                'editconfig_create_user_with_group_and_invalid_password',
                d,
                result)
        else:
            e = time.time()
    ...
    def editconfig_create_user_with_password_and_invalid_group(self, **kwargs):
        ...
        if result == 'PASS':
            ...
            csvOutput(
                'system-config',
                'editconfig_create_user_with_password_and_invalid_group',
                d,
                result)
        else:
            e = time.time()
    ...
    def editconfig_create_user_without_password_and_group(self, **kwargs):
        ...
        if result == 'PASS':
            ...
            csvOutput(
                'system-config',
                'editconfig_create_user_without_password_and_group',
                d,
                result)
    ...
    def editconfig_create_invalid_user_without_password_and_group(
        ...
        if result == 'PASS':
            ...
            csvOutput(
                'system-config',
                'editconfig_create_invalid_user_without_password_and_group',
                d,
                result)
    ...
    def editconfig_create_startup_mode_with_mode_preserve(self, **kwargs):
        ...
        if result == 'PASS':
            ...
            csvOutput(
                'system-config',
                'editconfig_create_startup_mode_with_mode_preserve',
                d,
                result)
        else:
            LOG.info('getting error from switch : FAIL\n\n')
    ...
    def editconfig_create_invalid_startup_mode_with_mode_preserve(
        ...
        if result == 'PASS':
            ...
            csvOutput(
                'system-config',
                'editconfig_create_invalid_startup_mode_with_mode_preserve',
                d,
                result)
        else:
            e = time.time()
    ...
    def editconfig_create_startup_mode_with_mode_volatile(self, **kwargs):
        ...
        if result == 'PASS':
            ...
            csvOutput(
                'system-config',
                'editconfig_create_startup_mode_with_mode_volatile',
                d,
                result)
        else:
            LOG.info('getting error from switch : FAIL\n\n')
    ...
    def editconfig_create_invalid_startup_mode_with_mode_volatile(
        ...
        if result == 'PASS':
            ...
            csvOutput(
                'system-config',
                'editconfig_create_invalid_startup_mode_with_mode_volatile',
                d,
                result)
        else:
            e = time.time()
    ...
    def editconfig_delete_all_users(self, **kwargs):
        ...
        """
        global xmlstr
        gvn_user = get_config_arg("login_credentials", "user_name")
        admin_user = get_config_arg("login_credentials", "user_name")
        user_user = get_config_arg("login_credentials", "users").split(",")
        ...
        if result == 'PASS':
            ...
            for user_ind in range(0, l):
                ...
                # user = SubElement(system_config, 'user',
                # {'ns:operation':'delete'})
                if 'admin' or name_val == admin_user or name_val == user_user[0] or name_val == user_user[1]:
                    continue
                else:
                    user = SubElement(
                        system_config,
            ...
            xmlout, result = self.get_rpc_request(xmlstr, kwargs['file_name'])
            result1, result2 = self.get_parsed_values(xmlout)
            if str(gvn_user) in str(result1) or str(user_user[0]) in str(result1) or str(user_user[1]) in str(result1):
                LOG.info('gvn_user : %s\n\n' % gvn_user)
    ...
    def delete_all_users(self, **kwargs):
        """ create xml for delete all users except admin
        using edit-config and get this modification using get-config
        then parsed this xml and compare parsed users with given users
        Arguments:
        ...
        if result == 'PASS':
            ...
            for user_ind in range(0, l):
                ...
                # user = SubElement(system_config, 'user',
                # {'ns:operation':'delete'})
                if name_val == gvn_user or name_val == 'admin':
                    continue
                else:
                    user = SubElement(
                        system_config,
            ...
            LOG.info('gvn_user : %s\n\n' % gvn_user)
            LOG.info('result1 : %s\n\n' % result1)
            if str(gvn_user) in str(result1) or 'admin' in str(result1):
                LOG.info('compare user : PASS\n')
                result = 'PASS'
            else:
                LOG.error('comparision failed : FAIL\n')
