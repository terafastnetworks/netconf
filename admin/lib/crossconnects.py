...
class CrossConnects:
    ...
    def cleanup_existing_connections(self, **kwargs):
        ...
        LOG.info('-----[ cleanup_existing_connections ]-----\n')
        config = Element(
            'config',
            {'xmlns:ns': "urn:ietf:params:xml:ns:netconf:base:1.0"})
        crossconnect = SubElement(
            config, 'cross-connects', {'xmlns': "http://www.polatis.com/yang/optical-switch",
                                       'nc:operation':
                                       "delete",
                                       'xmlns:nc': "urn:ietf:params:xml:ns:netconf:base:1.0"})
        xmlstr = tostring(config)
        LOG.info(
            '-----[ create xml for cleanup the all oxc connections ]-----\n\n%s' % self.prettify(xmlstr))
    ...
    def get_ingress_ports(self, **kwargs):
        ...
        LOG.info('xmlstr : \n%s'  % self.prettify(xmlstr))
        xmlout = self.get_rpc_request(xmlstr, kwargs['file_name'])
        req_egress_ports = str(kwargs['egress_ports'])
    ...
    def editconfig_replace_operation(self, **kwargs):
        ...
        s = time.time()
        LOG.info(
            '-----[ create xml for edit-config REPLACE operation ]-----\n')
    ...
    def editconfig_negative_case_with_invalid_ingress_port(self, **kwargs):
        ...
        #self.create_box(
        #    'test_editconfig_negative_case_with_invalid_ingress_port')
        f_name = kwargs['file_name'].split('.')
        self.create_box(f_name[0])
        # self.split_port_list(kwargs['ingress_ports'], kwargs['egress_ports'])
        req_ingress_ports = kwargs['ingress_ports']
        ...
        s = time.time()
        LOG.info(
            '-----[ create xml for edit-config invalid ingress port operation ]-----\n')
        xmlstr = self.create_xml('create')
        result, message = self.edit_config_opr()
        if result == 'PASS':
            LOG.info('-----[ create xml for get-config operation ]-----\n')
            ...
            csvOutput(
                'cross-connects',
                'editconfig_negative_case_with_ingress_port',
                d,
                result)
        else:
            e = time.time()
            ...
            csvOutput(
                'cross-connects',
                'editconfig_negative_case_with_invalid_ingress_port',
                d,
                'PASS')
            LOG.info(
                'getting error from switch using invalid ingress port : PASS\n\n')
        # e = time.time()
        # d = int(round((e - s)* 1000))
        # csvOutput('cross-connects',
        # 'editconfig_negative_case_invalid_oxc_connection', d, result)
    ...
    def editconfig_negative_case_with_invalid_egress_port(self, **kwargs):
        ...
        s = time.time()
        LOG.info(
            '-----[ create xml for edit-config invalid egress port operation ]-----\n')
        xmlstr = self.create_xml('create')
        result, message = self.edit_config_opr()
        if result == 'PASS':
            LOG.info('-----[ create xml for get-config operation ]-----\n')
            ...
            csvOutput(
                'cross-connects',
                'editconfig_negative_case_with_egress_port',
                d,
                result)
        else:
            e = time.time()
            ...
            csvOutput(
                'cross-connects',
                'editconfig_negative_case_with_invalid_egress_port',
                d,
                'PASS')
            LOG.info(
                'getting error from switch using invalid egress port : PASS\n\n')

        nose.tools.assert_equals('FAIL', result)
        nose.tools.assert_equals(kwargs['msg'], str(message))

    def get_oxc_using_invalid_egress_port(self, **kwargs):
        """create the xml with invalid egress port, then get the configuration using
        get-config operation, parsed the output and then compare the both ports list
        Arguments:
        ingress_ports		: valid ingress ports
        ...
        try:
            ...
            csvOutput(
                'cross-connects',
                'test_get_oxc_for_invalid_egress_port',
                d,
                'PASS')
            LOG.info('-----[ Error from switch ]-----\n\n%s\n' % err)
            LOG.info(
                'getting error from switch using invalid egress port : PASS\n\n')
            result = 'PASS'
        nose.tools.assert_equals('PASS', result)
    ...
    def get_oxc_using_invalid_ingress_port(self, **kwargs):
        """create the xml with invalid egress port, then get the configuration using
        get-config operation, parsed the output and then compare the both ports list
        Arguments:
        ingress_ports		: valid ingress ports
        ...
        for i in range(0, l):
            ...
            if a in ex_ingress_ports:
                pairs = SubElement(crossconnects, 'opsw:pairs')
                egress = SubElement(pairs, 'opsw:ingress')
                egress.text = str(a)
            else:
                pairs = SubElement(crossconnects, 'opsw:pairs')
                egress = SubElement(pairs, 'opsw:ingress')
        ...
        try:
            ...
            csvOutput(
                'cross-connects',
                'test_get_oxc_for_invalid_ingress_port',
                d,
                'PASS')
            LOG.info('-----[ Error from switch ]-----\n\n%s\n' % err)
            LOG.info(
            ...
            result = 'PASS'
        nose.tools.assert_equals('PASS', result)
        nose.tools.assert_equals(kwargs['msg'], str(err))
    ...
    def editconfig_negative_case_with_invalid_oxc_connection(self, **kwargs):
        ...
        #self.create_box(
        #    'test_editconfig_negative_case_for_invalid_oxc_connection')
        f_name = kwargs['file_name'].split('.')
        self.create_box(f_name[0])
        # self.split_port_list(kwargs['ingress_ports'], kwargs['egress_ports'])
        req_ingress_ports = kwargs['ingress_ports']
        ...
        s = time.time()
        LOG.info(
            '-----[ create xml for edit-config invalid oxc operation ]-----\n')
        xmlstr = self.create_xml('create')
        result, message = self.edit_config_opr()
        if result == 'PASS':
            LOG.info('-----[ create xml for get-config operation ]-----\n')
            ...
            csvOutput(
                'cross-connects',
                'editconfig_negative_case_invalid_oxc_connection',
                d,
                result)
        else:
            e = time.time()
            ...
            csvOutput(
                'cross-connects',
                'editconfig_negative_case_with_invalid_oxc_connection',
                d,
                'PASS')
            LOG.info(
                'getting error from switch using invalid ingress and egress port : PASS\n\n')
