...
class Set_Current_DateTime:
    ...
    def compare_list(self, xmlData):
        ...
        for t in ct:
            for i in range(0, l):
                try:
                    ...
                    set_curr_time_tag_value = str(
                        set_curr_time_tag.childNodes[0].data)
        ...
        if str(curr_time) == str(set_curr_time_tag_value):
            LOG.info(
                'compare both existing datetime and parsed datetime : PASS\n')
            return p
        else:
    ...
    def set_and_get_current_datetime(self, **kwargs):
        ...
        username = get_config_arg("login_credentials", "host")
        ...
        #    "date -u +'%Y-%m-%dT%H:%M:%S.%s%z'")

        #t = stdout.readlines()
        login = pexpect.spawn ('ssh %s@%s' % (username, host))
        time.sleep(5)
        #login.expect (':')
        #time.sleep(5)
        #login.sendline ('abcd123')
        #time.sleep(5)
        print login.expect ('password:')
        time.sleep(5)
        print login.sendline ('P0lat1s')
        time.sleep(5)
        print login.expect ('#')
        time.sleep(5)
        print login.sendline ("date -u +'%Y-%m-%dT%H:%M:%S.%s%z'")
        time.sleep(5)
        login.expect ('#')
        time.sleep(5)
        t = login.before 
        ...
        curr_time = '2016-09-20'
        curr_time = str(datetime.datetime.now())
        LOG.info('-----[ create xml for set operation ]-----\n')

        xmlstr = """<set-current-datetime xmlns="http://www.polatis.com/yang/optical-switch"><current-datetime>2016</current-datetime></set-current-datetime>"""
        s_curr_time = Element('set-current-datetime', {'xmlns':"http://www.polatis.com/yang/optical-switch"})
        c_dt = SubElement(s_curr_time, 'current-datetime')
        c_dt.text = '2016-11-25T13:04:16.858894-00:00'

        #xmlstr = tostring(s_curr_time)
        LOG.info('%s\n' % xmlstr)
        try:
            xmlout = sw_mgr.rpc(xmlstr)
            print "checking ........... xmlout", xmlout
            print '\n\n'
    ...
    def set_invalid_datetime(self, **kwargs):
        """create xml to get current datetime info then parse get switch output xml
        , compare it with existing values.Finally save the output in final.csv file
        Arguments:
        file_name             : any file name
        ...
        #xmlstr = """<set-current-datetime xmlns="http://www.polatis.com/yang/optical-switch"><current-datetime>2015-13-16</current-datetime></set-current-datetime>"""
        #xmlstr = xmlstr.encode("utf-8")
        #print "type : ", type(xmlstr)
        #LOG.info('%s\n' % str(xmlstr))
        try:
            xmlout = sw_mgr.rpc(xmlstr)
            print "xmlout : ", xmlout
            print '\n\n'
            LOG.info('-----[ Able to set invalid datetime ]----- FAIL\n')
            result = 'FAIL'
        except Exception as err:
            result = 'PASS'
            LOG.info('-----[ Getting error from switch ]-----\n\n%s\n\n' % err)
        nose.tools.assert_equals('PASS', result)
        nose.tools.assert_in('/rpc/set-current-datetime/current-datetime: "2015-13-16', err)
